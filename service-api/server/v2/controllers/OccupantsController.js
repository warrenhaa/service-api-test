...
class OccupantsController {
  ...
  static async deleteOccupantInvite(req, res) {
    const { id } = req.params;
    const inviteToDelete = await OccupantService.deleteOccupantInvite(id, req);
    if (inviteToDelete) {
      util.setSuccess(200);
    } else {
  ...
  static async deleteOccupant(req, res) {
    ...
    const occupantToDelete = await OccupantService.deleteOccupant(req, id, email); if (occupantToDelete) {
