...
const addOccupantsPermissionsMetadataRules = () => [
  ...
  body('key', 'key has exceeded the max character length 255').exists().isString().notEmpty()
    .isLength({ max: 255 }),
...
const disableGeofencingRules = () => [
  query('occupant_permission_id', 'occupant_permission_id missing').exists().notEmpty(),
  query('occupant_permission_id', 'invalid occupant_permission_id').isUUID(),
  query('gateway_id', 'gateway_id missing').exists().notEmpty(),
  query('gateway_id', 'invalid gateway_id').isUUID(),
];
