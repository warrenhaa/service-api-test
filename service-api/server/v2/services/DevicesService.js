...
import OccupantsPermissionsMetadataService from './OccupantsPermissionsMetadataService'
...
class DevicesService {
  ...
  static async updateDevice(req) {
    ...
    if (deviceToUpdate) {
      ...
      if (req.body.devices_metadata) {
        ...
        var autoSwitchStatus = metadata.filter(obj => {
          return obj.key == 'auto_switch_status'
        })
        if (req.occupant_id && autoSwitchStatus && autoSwitchStatus.length > 0) {
          autoSwitchStatus = autoSwitchStatus[0];
          ...
          if (getMetaData == null || (getMetaData && getMetaData.value !== autoSwitchStatus.value)) {
            const occupantsPermissions = await database.occupants_permissions.findOne({
              ...
              throw Error(JSON.stringify({ error: '160026' }));
            });

            if (occupantsPermissions) {
              if (occupantsPermissions.access_level != 'O') {
                throw Error(JSON.stringify({ error: '160059' }));
              }
            } else {
              throw Error(JSON.stringify({ error: '160026' }));
            }
          ...
          await OccupantsPermissionsMetadataService.manageMyStatus(autoSwitchStatus.value, req.body.company_id, id);
        }
        ...
        if (getSingleControlAndDevices && getSingleControlAndDevices.single_control_devices && getSingleControlAndDevices.single_control_devices.length > 0) {
          ...
          if (singleControlDevicesArray.length > 1) {
            ...
            if (exDefaultDevices.length >= 1) {
              ...
              const url = `https://${host}/api/v2/schedules/device_schedules?ref=${ref}`;
          ...
          if (!deviceReferenceObj) {
            let gatewayExists = await database.devices.findOne({
              where: { id: getSingleControlAndDevices.gateway_id },
              raw: true,
            }).catch(() => {
              const err = ErrorCodes['800019'];
  ...
  static async deleteGateway(command, gateway_id, company_id, occupant_id, occupant_email, request_id, code, source_IP) {
    ...
    if (command == 1) {
      ...
      const device_code = checkGatewayIdExists.device_code;
      const input = {
        gatewayId: gateway_id,
        deviceCode: device_code,
        companyId: company_id,
        occupantEmail: occupant_email,
        occupantId: occupant_id,
      };
      const job = await jobsService.createJob(Entities.jobs.event_name.occupant_delete_gateway_job, input, company_id, occupant_id, occupant_id, null, request_id)
        .then(async (resp) => resp)
        .catch((err) => {
          throw (err);
        });
  ...
  static async gatewayCameraList(gatewayid, gateway_code, occupant_id, user_id) {
