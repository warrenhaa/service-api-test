  ...
  static getAlerts(sliderObj, sharedDeviceCodeList) {
    return new Promise(async (resolve, reject) => {
      ...
      if (type == 'gateway') {
        ...
        timezone = sliderObj?.gateway?.timezone
      ...
      let attributes =  ['id', 'alert_type', 'alert_code', 'is_viewed', 'severity', 'created_at']
      if(process.env.ALERTS_IN_GATEWAY_TIMEZONE == true || process.env.ALERTS_IN_GATEWAY_TIMEZONE == "true"){
        if(timezone){
          attributes = [
            'id',
            'alert_type',
            ...
            'is_viewed',
            'severity',
            [database.Sequelize.literal(`"device_alerts"."created_at" AT TIME ZONE '${timezone}'`), 'created_at']
          ]
          }
      }
